...
int main()
{
    ...
    scanline(x0+xin,y0-yin,fill_color,boundary_color);
...
void scanline(int x,int y,int fcolor,int bcolor)
{
    int xl,xr;
    if ((getpixel(x, y) != fcolor) && (getpixel(x, y) != bcolor))
    {
        ...
        while((getpixel(--xl, y) != bcolor))
              {
                   // printf("\n%d,%d",xl,y);
                    putpixel(xl,y,fcolor);
              }
        while((getpixel(++xr, y) != bcolor))
              {
                   // printf("\n%d,%d",xr,y);
                    putpixel(xr,y,fcolor);
              }

        scanline(x,y+1,fcolor,bcolor);
        scanline(x,y-1,fcolor,bcolor);
