...
int main()
{
    ...
    printf("Enter sxx,sxy,syx,syy");
    scanf("%d%d%d%d",&sxx,&sxy,&syx,&syy);
    printf("test\n\n");
    for(i=0;i<n-1;i++)
    {

    reflect(sxx,sxy,syx,syy,x[i],y[i],x[i+1],y[i+1]);}
...
void reflect(int sxx,int sxy,int syx,int syy,int x1,int y1,int x2,int y2)
{
  ...
  line_draw(x1,y1,x2,y2,300,300,7);
