...
void bfsl(int x,int y,int xmin,int xmax,int fc,int bc,int ,int);
...
int main()
{
    ...
    printf("Enter the boundary colour ,fill colour and interior points x and y");
    scanf("%d %d %d %d",&bc,&fc,&xi,&yi);
      while(z!=(n-1))
    {
        line_draw(x[z],y[z],x[z+1],y[z+1],x0,y0,bc);
        z++;
    }
    ...
    for (c = 1; c < n; c++)
  {
    ...
    bfsl(xi+x0,y0-yi,min+x0,maximum+x0,fc,bc,x0,y0);
    delay(40000);
...
void bfsl(int x,int y,int xmin,int xmax,int fc,int bc,int x0,int y0)
{
    ...
    if((getpixel(x,y)!=fc)&&(getpixel(x,y)!=bc))
    {
     ...
     printf("%d %d  \n",xl+1,y+1);
     bfsl(xl+1,y+1,xmin,xmax,fc,bc,x0,y0);
     printf("return \n");
     bfsl(xl+1,y-1,xmin,xmax,fc,bc,x0,y0);}
}
