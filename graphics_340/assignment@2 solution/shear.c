...
void shear(int shx,int shy,int xref,int yref,int x1,int y1,int x2,int y2);
...
int main()
{
    ...
    printf("Enter shx,shy,xref,yref");
    scanf("%d %d %d %d",&shx,&shy,&xref,&yref);
    printf("test");
    for(i=0;i<n-1;i++)
    {

    shear(shx,shy,xref,yref,x[i],y[i],x[i+1],y[i+1]);}
...
void shear(int shx,int shy,int xref,int yref,int x1,int y1,int x2,int y2)
{
  ...
  line_draw(x1,y1,x2,y2,300,300,2);
