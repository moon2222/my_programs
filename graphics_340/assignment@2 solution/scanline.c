...
int main()
{
    int gd=DETECT,gm,min=0,maximum=0,c, location,loc, pp,  o,m,counter=0;
    int p[100];
    double t,m1;
     ...
     for (c = 1; c < n; c++)
  {
    ...
    while(pp<maximum)
    {
        for(m=0;m<n-1;m++)
        {
            ...
            if((pp>=y[m]&&pp<y[(m+1)%n])||(pp>=y[(m+1)%n]&&pp<y[m]))
            {
             //if((x[(m+1)]-x[m])==0){x[m+1]=x[m+1]+1;}
            t=(pp-y[m]);
            m1=((double)y[(m+1)%n]-y[m])/(x[(m+1)%n]-x[m]);
            m1=1/m1;
            t=t*m1 + x[m];
            printf("%d %d %d %f %f   ",y[m],y[(m+1)%n],pp,t,m1);
            p[counter++]=t;
            }
            /*else
               {
                   t= x[m];
            printf("%d %d %d %f %f inside ",y[m],y[(m+1)%n],pp,t,m1);
            p[counter++]=t;
        ...
        for(o=0;o<counter;o=o+2)
        {
            ...
            delay(100);
            line_draw(p[o]+1,pp,p[(o+1)%counter]-1,pp,x0,y0,4);
            //printf("%d %d",p[0],p[(o+1) %counter]);
